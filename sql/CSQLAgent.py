...
import sql.sql_connect_vars as sql_config
...
class CSqlAgent(csql_eng):
    ...
    @staticmethod
    def set_sql_data_local(data_dict: dict):
        sql_config.db_standart_connect_params = data_dict.copy()

    @staticmethod
    def set_sql_data_line(data_dict: dict):
        sql_config.db_assembly_connect_params = data_dict.copy()
    ...
    def connect_to_db(self, connect_db_type: cenum.CONNECT_DB_TYPE) -> bool:
        if self.__connect_handle is False:
            ...
            if connect_db_type == cenum.CONNECT_DB_TYPE.LINE:
                self.set_connect_data(sql_config.db_assembly_connect_params)
            elif connect_db_type == cenum.CONNECT_DB_TYPE.LOCAL:
                self.set_connect_data(sql_config.db_standart_connect_params)
            else:
            ...
            connect_handle = self.sql_connect()
